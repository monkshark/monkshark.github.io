...
즉 `MM`은 월이 아니라 **분**이다. `2026-00-30`은 "분이 00", `2026-55-14`는 "분이 55"였던 것. 글이 작성된 시각의 분이 월 자리에 박혀 있었다.
...
원인은 마커였다. 액션은 `START` 주석 **바로 뒤**에 내용을 이어 붙인다. 내 템플릿이 `...$newline`으로 끝나니, 결과가 이렇게 됐다.
...
첫 글이 `<!-- ... -->`와 **같은 줄**에 붙어 있다. Markdown에서 리스트 항목의 `-`는 줄 맨 앞에 와야 하는데, 앞에 주석이 있으니 첫 줄은 리스트로 인식되지 않는다.

해결은 템플릿 **앞쪽**에 줄바꿈을 두는 것. 그러면 첫 글이 주석 다음 줄에서 시작한다.
