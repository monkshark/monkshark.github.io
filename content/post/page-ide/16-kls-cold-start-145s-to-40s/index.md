...
수정의 모양은 이렇다. 워크스페이스를 걸을 때, 그게 **하나의 루트 빌드**인지를 먼저 본다. 맞으면 build file 마다 resolver 를 까는 대신 루트 build file 하나에 resolver 하나만 깐다. 그 하나가 루트에서 Gradle 을 한 번 띄우고, init script 의 `allprojects` 가 나머지를 다 데려온다.
